...
| 2 | `## Role taxonomy` (20–121) | **30-role map deleted.** Kept only what `director-*`, `principal-*`, `workflow-*`, `persona-*`, `instructor-*`, `eng-lead-*` (and the newer `<family>-<posture>-<scope>` postures) mean as *kinds*, plus "stay in your lane". **`creative-*` dropped.** The source defined a `creative-*` kind ("own creative and media-production judgment … They direct; they do not render, edit, or publish"); it is not in the kept map. One `creative-*` agent still ships (`kai-personal/agents/creative-video-director.agent.md`), and its operative half survives in that agent's own body ("You do not render or edit") plus the human-gates list, so no operative rule was lost — only the taxonomy entry. Note the reverse: `persona-*` and `eng-lead-*` are *additions* the source taxonomy did not define. |
