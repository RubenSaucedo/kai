...
**Files:**
- Modify: `scripts/generate-catalog.mjs` (`CATEGORIES`)
...
- [ ] **Step 1: Update the skill registries**

In `scripts/generate-catalog.mjs`, replace the four deleted skill ids in `CATEGORIES` with the eight new ones. Coverage is enforced — every skill must be filed under exactly one heading or the build fails.
