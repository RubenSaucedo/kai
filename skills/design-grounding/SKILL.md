...
3. **No reference and no accessible repo tokens** (e.g. an external product, or a
   ...
   `work-coordination`, not by spawning it). The designer **does not** free-roam
...
## Missing-scale bubble-up — wired to `scope-discipline`
...
`design-system.md` and emit the `scope-discipline` `PROPOSAL` payload, then keep
