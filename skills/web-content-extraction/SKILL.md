...
`generate-audio` skill so the user can listen on the go, but the skill
...
Sister skill to `web-evaluation`: that one is for evaluating a UI;
...
- UI evaluation runs — use `web-evaluation` (the QA / UX agents call
  that one).
...
working output, and `generate-audio` reads the markdown straight from that
path.
...
> Generated by `web-content-extraction`. For audio: run
> `generate-audio` on this folder. Knowledge-check questions live in
> `questions.md`.
