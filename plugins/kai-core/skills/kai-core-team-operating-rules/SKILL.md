...
- `principal-product-designer` owns interaction design for approved needs,
  grounded in the app's design system (`kai-core-design-grounding`) and presented as
...
- `principal-demand-generation` owns demand-generation strategy, campaigns, and
  lifecycle programs; it inherits `kai-core-content-grounding`, routes positioning/claims
...
`.kai/state/ACTIVE.md`. Load a matching north star only when the current
target fits its scope. When loaded, ground work in its mission, current
milestones, success measures, and non-negotiables.

Any agent that changes product/code inherits `kai-core-scope-discipline`: implement
...
Design and frontend agents also inherit `kai-core-design-grounding`, and the designer
...
For coordinated work, `.kai/state/items/<item-id>.md` is authoritative and
`.kai/state/BOARD.md` is a derived index.
...
decisions live under `.kai/state/initiatives/<slug>/artifacts/`; recorded operator
...
An item moves `in-review -> release-ready` only through `kai-core-definition-of-done`.
...
Use `kai-core-peer-communication`. Address roles, not people. Decision-grade judgment
...
every skill it names exists, that every agent inherits `kai-core-team-operating-rules`,
that every coordinating role also inherits `kai-core-workspace-conventions`, and that any
...
`kai-core-workspace-conventions` for root resolution and artifact placement,
`kai-core-work-coordination` for item state, leases, dependencies, and handoffs,
`kai-core-scope-discipline` for the classify-before-adopt gate, `kai-core-definition-of-done` for
the release gate, and `kai-core-peer-communication` for message shape. When this summary
