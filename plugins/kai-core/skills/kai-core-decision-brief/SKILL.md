...
decide this — in one place."* The agenda (`kai-core-personal-agenda`) tells the operator
...
inward complement to `kai-core-executive-consultation`:

| Skill | Direction | Trigger |
|---|---|---|
| `kai-core-executive-consultation` | **outward** — "go ask the team" | operator wants insight/data the records don't hold |
| `kai-core-decision-brief` | **inward** — "package what's already waiting on me" | a decision is already pending on the operator |

The two compose: a decision brief that is missing a role's position spawns a
narrow `kai-core-executive-consultation` to fill exactly that gap, then folds the
...
4. **Fill only the gaps.** If a materially affected role has not weighed in, or a
   load-bearing fact is missing, run one `kai-core-executive-consultation` scoped to
...
4. **Fill gaps, don't re-litigate.** Consult only for a genuinely missing
   position or fact; reuse `kai-core-executive-consultation` and its provenance.
