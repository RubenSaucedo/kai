...
through `scripts/generate-audio.ps1` in the **kai-core provider root**. The
wrapper resolves Lectoria in this order: `LECTORIA_BIN`, this pack's
`node_modules/.bin/lectoria`, then PATH. **Lectoria is pinned in kai-core's
generated `package.json` and `package-lock.json`**, projected from the canonical
root dependency metadata.

Copilot installs plugin files but does **not** run npm. Base kai-core behavior
needs no npm install; audio does. Before the first audio run, or after a plugin
update removes `node_modules`, run:
...
**Resolving the script path.** `<kai-core-plugin>` below is the provider root:
start from this loaded skill's base directory
`<kai-core-plugin>/skills/kai-core-generate-audio/` and go up two directories.
Invoke the resulting absolute
`<kai-core-plugin>/scripts/generate-audio.ps1`. Never derive it from a calling
department's plugin root, scan the Copilot cache, traverse to a sibling
directory, or hard-code a checkout like `C:\src\...`.
...
[runs: pwsh <kai-core-plugin>/scripts/generate-audio.ps1]
</example>

<example>
Context: User wants to hear a single README.
user: "make me an audio version of README.md in spanish"
assistant: [runs: pwsh <kai-core-plugin>/scripts/generate-audio.ps1 -Source ./README.md -Lang es]
</example>

<example>
Context: User wants verbatim narration (no LLM expansion) of a course chapter.
user: "verbatim audio of chapter-3.md"
assistant: [runs: pwsh <kai-core-plugin>/scripts/generate-audio.ps1 -Source ./chapter-3.md -Style verbatim]
</example>
...
assistant: [runs: pwsh <kai-core-plugin>/scripts/generate-audio.ps1 -Source .kai/runs/learn/<goal-slug>/<NN>-extract-<source-slug>/raw -Lang es]
</example>

## How

1. **Install only when audio is requested.** Resolve `<kai-core-plugin>` from
   this skill's provider root and run `npm ci --prefix "<kai-core-plugin>"`.
   Copilot does not run this during plugin installation. Rerun it after an
