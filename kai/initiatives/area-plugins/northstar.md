...
milestones:
  - id: allowlist-repair
    outcome: The repo's tool-allowlist contract is grounded in measured host behaviour instead of hand-maintained guesswork, every kai agent declares only names the live Copilot CLI accepts, the repeated "Unknown tool name in the tool allowlist" warnings are gone, and no agent loses a capability to achieve it. Led by the conformance probe, not by a rename.
    acceptance:
      ...
      - No capability is stripped to quiet a log. The smallest warning-free replacement follows from probe evidence and not before; where a warned name has no accepted equivalent that preserves the capability, the record says so, and a replacement that removes capability is a capability-loss disclosure and a steward call, not an editorial one.
      ...
      - The warnings are reported gone against a stated observation method. A claim of absence made without a run is `reported`, never `observed`, and must be labelled that way.
...
|---|---|---|
| `allowlist-repair` | 2 | `host-tool-conformance` **ready** (lease `principal-swe-infra`, in flight); `tool-allowlist-fix` **blocked** on `Q-…-01`, `resume_state: ready` |
