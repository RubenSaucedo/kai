...
PR #174 merged to `main` as `b6db547c41b606c92e78e9d91fab82c554fc7d3d`;
main validation run `33028413182`, job `98375047081`, succeeded. The
disposable consumer repository was deleted after evidence preservation.
`first-pack-extracted` is closed at 4/4 and active scope is now
`five-pack-split-shipped`. The sole ready and executable item is
`pack-split-pack-dependency-manifests` (priority 10, owner/lease clear, next
`principal-swe-infra`; host-gates dependency met). Onboarding and releases
12a/12b/12c remain proposed. Release 12b remains **NO-GO**; packs are
unpublished and marketplace topology and versions are unchanged.

**Dependency-contract build 2026-08-26-1818.**
`pack-split-pack-dependency-manifests` is `in-progress` under
`principal-swe-infra` lease `psinfra-1818-dm65`. Generated core and personal
packs now project pack-local npm manifests and the root-pinned Lectoria lock
graph; emitted imports require same-pack declarations and lock records. A
temporary core + personal installation accepted both projected lockfiles,
